...
int main(void) {
    ...
    if (numero < 0) { printf("Error: El factorial solo está definido para números no negativos.\n");
        return EXIT_FAILURE;
    }
    if (numero > 20) {printf("Advertencia: El factorial de %d es muy grande y probablemente cause un resultado incorrecto por desbordamiento (overflow).\n", numero);}
